...
class HttpFetch(RetrievalProvider):
    def __init__(self, cache_dir: Path = OPS_CACHE, timeout: float = 20.0,
                 user_agent: str = "provider-taxonomy/0.1 (+grounding-gate)"):
    ...
    def fetch(self, url: str) -> FetchedPage:
        ...
        cached.write_text(json.dumps({"status": resp.status_code, "text": text}), encoding="utf-8")
        return FetchedPage(url=url, status=resp.status_code, text=text, content_hash=content_hash(text))
